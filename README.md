...
#### 1. 生成班级邀请码（教师专用）
```bash
curl -X POST "http://localhost:8080/api/class/generate-invite-code?classId=1" \
  -b cookies.txt
```
**响应**:
```json
...
```
**注意**: 邀请码为25位大小写字母+数字组合，安全性更高。

#### 2. 通过邀请码加入班级
```bash
curl -X POST "http://localhost:8080/api/class/join-by-code?inviteCode=A3f9K2mN7pQ1xR5tY8wZ4bC6d" \
  -b cookies.txt
```
**响应**:
```json
...
```
**注意**: 通过邀请码加入班级无需审核，直接以学生身份入班。

### 3. 转让班级所有权
```bash
curl -X PUT "http://localhost:8080/api/class/transfer-ownership?classId=1&newOwnerId=1003" \
  -b cookies.txt
```
**响应**:
```json
...
所有列表接口均支持分页查询，使用 `pageNum` 和 `pageSize` 参数：

#### 1. 查询我的班级列表（分页）
```bash
curl -X GET "http://localhost:8080/api/class/mylist?pageNum=1&pageSize=10" \
  -b cookies.txt
```

#### 2. 查询班级成员列表（分页）
```bash
curl -X GET "http://localhost:8080/api/class/members?classId=1&pageNum=1&pageSize=20" \
  -b cookies.txt
```

#### 3. 查询作业列表（分页）
```bash
curl -X GET "http://localhost:8080/api/works/list?pageNum=1&pageSize=20" \
  -b cookies.txt
```

#### 4. 查询作业提交列表（分页）
```bash
curl -X GET "http://localhost:8080/api/work/submissions?workId=1&pageNum=1&pageSize=20" \
  -b cookies.txt
```
